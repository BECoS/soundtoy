...
function initView () {
  ...
  for (var voice = 0; voice < totalVoices; voice++) {
    ...
    for (var cube = 0; cube < length; cube++) {
      row += 
        "<figure class='inactive square_" + gmodel.getNoteFromRow(voice) + "'>" +
...
function writeNoteName(square) {
  for (var i = 0, A = $(square).attr('class').split(/\s+/); i < A.length; i++) {
    if (A[i].match(/square/) !== null) {
      $(square).html(A[i].match(/_([a-zA-Z]\d+)/)[1]);
    }
  }
}
